...
> **SQLite note:** SQLite is always standard auth — it is file-based and does not support network IAM auth. If the user needs SQLite, go directly to Section A.
...
- Signals found → go to [Section A-IAM: SQLAlchemy + AWS IAM](#section-a-iam-sqlalchemy--aws-iam)
- No signals → ask: *"Is this project for a regulated industry (e.g. healthcare, finance, government) or does it need to connect to AWS RDS using IAM authentication rather than a password?"*
  - **Yes** → [Section A-IAM: SQLAlchemy + AWS IAM](#section-a-iam-sqlalchemy--aws-iam)
  - **No** → [Section A: SQLAlchemy (standard)](#section-a-sqlalchemy-sql)
  - **Not sure** → [Section A: SQLAlchemy (standard)](#section-a-sqlalchemy-sql) — a migration path is available later
...
## Section A-IAM: SQLAlchemy + AWS IAM

> This section covers the AWS IAM authentication overlay. Follow Section A steps A1 (packages), A2 (base.py), and A5 (alembic init + alembic.ini edit) only — **skip A3, A4, and the alembic env.py DATABASE_URL lines**. Then apply the IAM steps below in place of those skipped steps. Continue from A6 onwards after applying these changes.
...
### A-IAM.1 — Additional package

In addition to `sqlalchemy` and `alembic`, add to `requirements.txt`:

```
boto3
```

### A-IAM.2 — Replace `src/database/session.py` with IAM session
...
### A-IAM.3 — Replace `DATABASE_URL` with IAM fields in `src/core/config.py`

Replace the `DATABASE_URL` field in `APISettings`:
...
> **These IAM fields replace `DATABASE_URL`** — remove `DATABASE_URL` from `APISettings` when using IAM auth.
...
### A-IAM.5 — Continue from Section A

Follow steps A6 onwards (create a model, generate first migration, usage) from Section A — no further changes needed.
...
- Keep credentials (`DATABASE_URL`, `MONGODB_URL`, or IAM host/user fields) in `src/.env` — NEVER hardcode production credentials.
...
> See [Section A-IAM](#section-a-iam-sqlalchemy--aws-iam) for the complete AWS IAM authentication setup for SQLAlchemy. The IAM path for Beanie/MongoDB uses `pymongo[aws]` — install it and use the `MONGODB-AWS` connection string shown in the Rules above.

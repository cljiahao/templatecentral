...
# templateCentral Skill Audit

Perform a comprehensive audit of all templateCentral skills. Follow every step in order — do not skip steps or combine them.

**Announce at start:** "Running shared-audit: ecosystem research → mechanical checks → semantic review."
...
- [ ] `skills/shared-audit/SKILL.md` ← this file; check it is still accurate
...
If the issue requires judgment (context-dependent, not detectable by grep alone), add a checklist item to the relevant section of this file (`skills/shared-audit/SKILL.md`):
...
```bash
git add scripts/lint-skills.sh skills/shared-audit/SKILL.md

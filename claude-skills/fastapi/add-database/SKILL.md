---
name: add-database
description: Use when the user wants to add a database to a FastAPI project — SQLAlchemy for SQL databases (PostgreSQL, MySQL, SQLite) or Beanie for MongoDB.
---
...
Edit `alembic.ini` — set `prepend_sys_path` so Alembic can find `src/` modules, and set the database URL:

```ini
prepend_sys_path = src
sqlalchemy.url = sqlite:///./src/app.db
```

Edit `alembic/env.py` (this is illustrative — keep the full boilerplate generated by `alembic init`):

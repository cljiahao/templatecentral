...
- **Hardened the seeded harness guards** in `skills/scaffold/shared/harness-kit.md` (mirrored in the OpenCode adapter `adapters/opencode/templatecentral.plugin.js`); these guards are seeded into every scaffolded/migrated project, so the fix propagates on next scaffold/re-sync:
  ...
  - **`settings.json` `.env` Read-deny is now a catch-all** — the hardcoded enumeration of `.env.*` suffixes is replaced with `Read(**/.env.*)` (plus `Read(.env)`/`Read(**/.env)`), with `.env.example`/`.env.default` added to `permissions.allow`. NOTE: per current Claude Code docs, `deny` is evaluated before `allow` and cannot carry allowlist exceptions, so the catch-all also read-blocks the two committed templates; they remain writable via `protect-files.sh` and non-secret (gitleaks-allowlisted). Applied to both the TS and FastAPI settings blocks.

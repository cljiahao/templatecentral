...
- **Hierarchical README governance — per-folder documentation for every scaffolded + migrated project.** Every project directory now gets a structural `README.md` (short `Purpose`/`Contents`, plus a `Connectivity` section on folders that contain subfolders, capped at 2-4 sentences to avoid the verbosity that measurably hurts agent task performance — ETH Zurich). An opt-in Azure DevOps Code Wiki mode (`adoWiki`, persisted in `.claude/harness.json`) additionally maintains a `.order` file per folder, since ADO's "publish repo to wiki" renders a parent folder blank if it has only subfolders and no file of its own.
  - **Content generation SSOT** — new `skills/scaffold/shared/documentation-kit.md`: the template, the folder-enumeration `find` (pruning dependency/build/VCS dirs, and additionally filtering through `git check-ignore` for project-specific patterns a hardcoded list can't anticipate), and the ADO opt-in read/write (with a documented non-interactive default of `false` for headless runs). Loaded by `scaffold` (automatically, via a new `harness-kit.md` Step E3), `migrate` (Step 4f-1c, plus a re-run after Step 4g's flat-to-directory skill conversion creates new folders), and a new on-demand `templatecentral:add (documentation)` capability (aliases `docs`, `readme`) for backfilling already-harnessed existing projects.
...
- **Lint:** `check_no_bare_nextjs_route_handlers` (catches skill examples showing a bare `export async function GET/POST/...` App Router handler instead of the required `withLogging()` wrap) and `check_no_bare_pytest_invocation` (catches a bare `pytest` invocation instead of `python -m pytest`, which resolves via PATH and silently breaks or picks the wrong interpreter when `.venv` isn't activated).

### Fixed

Three passes of verification before merge: a full `/tc-audit` sweep of all 67 stack skill files; an end-to-end smoke test that actually scaffolded and ran all 4 stacks from this branch's code (not the installed plugin); and a follow-up audit pass over everything the smoke test touched.

**End-to-end smoke test** — scaffolded all 4 stacks, ran install/typecheck/lint/tests/build, started each app, and exercised `readme-coupling` live via real commits. The README-governance feature itself worked correctly in all 4; these bugs were in the surrounding scaffold/harness pipeline and only manifest when a scaffold is actually run, not on code review alone:
...
- **`## Skill capture` never got seeded, for any stack.** Step G's append gated on `## AI Harness` already being present — true for every stack template — so the whole tail fragment (including `## Skill capture`, never pre-embedded anywhere) was always skipped. Now gates each of the three tail sections independently; mirrored the same fix into `migrate/general/implementation.md`.
...
- **`documentation-kit.md`'s folder-enumeration `find`** was missing `build/`, `.stryker-tmp`, `.mutmut-cache` from its dependency/build-output prune list. The `Contents` bullet list also no longer includes prune-list/gitignored entries verbatim (previously a root README could list `node_modules/` as project structure), and the root-README special case now handles "no root README exists at all," not just "exists but lacks a Structure section."
- **`.harness-base` exclude pattern** hardened from `*` to `**` in `harness-kit.md` (both patterns verified working against the real lefthook 2.1.10 binary; `**` makes the "any depth" intent explicit rather than relying on implementation-specific matcher behavior).
- **Bare `pytest` invocation, systemic** — 14 FastAPI skill files (`add/auth`, `add/database/python/*`, `add/endpoint`, `add/error-handling`, `add/integration`, `add/pagination`, `add/test`, `migrate/database`, `migrate/nextjs-backend-extraction/fastapi.md`, `scaffold/fastapi/source-files.md`, `standards/code-standards`, `standards/validation-patterns`, `test/implementation.md`) invoked bare `pytest` in their Validate sections instead of `python -m pytest`, the same failure mode already fixed in the scaffold's own lefthook commands. All now use `python -m pytest`, matching the `python -m pyright` convention used everywhere else in the FastAPI skills.
...
- **Audit skill's own checklist had drifted stale** — `audit/implementation.md`'s FastAPI-specific checks still cited Starlette "current stable 1.2.1" (now 1.3.1) and a `@nestjs/platform-fastify` floor of `≥11.1.19` (a later trailing-slash auth-bypass advisory raised the correct floor to `≥11.1.27`); its Step 2 cross-stack file inventory was also missing `documentation-kit.md` and `add/documentation/implementation.md`.
- **`scripts/validate-manifest.sh`** — `check_skill_frontmatter`'s `find ... -not -path '*/.*'` excluded every `SKILL.md` whenever the repo itself is checked out under a dot-directory (e.g. any `.claude/worktrees/<name>/` git worktree — the standard, documented Claude Code worktree layout), because the exclusion matched the ancestor `.claude` path segment, not just hidden subdirectories inside `skills/`. Fixed to filter on the path *relative to* the skills directory.
- 20+ files across `add/`/`standards/` had trailing inline comments moved to their own line, matching this repo's comment doctrine.
- Verified clean (no changes needed): NestJS `@nestjs/platform-fastify` floor, Starlette "current stable" doc text, React Router v8 migration, and the `nestjs-drizzle.md` missing-`@rc` bug all matched current guidance on inspection.

**Final fresh re-scaffold, all 4 stacks, at the fully-fixed state above** — every prior fix so far had been verified either via targeted checks against already-scaffolded projects or via diff/lint review; this pass scaffolded all 4 stacks completely fresh from scratch to confirm the *combined* final state actually works end to end (install, lint, build, run, first commit, `readme-coupling` live in both directions). Next.js and Vite+React came back fully clean. Two more real bugs surfaced:

- **FastAPI's scaffold never ran `git init`** — the only one of the 4 stacks that didn't (nestjs/nextjs/vite-react all do, before dependency install). `documentation-kit.md`'s gitignore-respect filter (added above) intentionally no-ops when no git repo exists yet, so it silently wrote a `README.md` into `src/log/` — a directory the scaffold's own `.gitignore` excludes — reproducing the exact failure the filter exists to prevent, since git would never track that file. Added `git init` to FastAPI's Step 4, matching the other three stacks.
- **The TS-stack CI `quality` job's coverage step wrote no coverage file.** `pnpm test -- --run --coverage` silently produced no `coverage/cobertura-coverage.xml`, because every TS stack's `test` script already resolves to `vitest --run`/`vitest run`, so pnpm's `--` separator appended a redundant second `--run` alongside `--coverage` — Vitest's CLI parser either rejects the duplicate flag outright or (when routed through the npm script) drops `--coverage` as an inert positional filter. The very next CI step (`diff-cover` changed-line coverage) would fail with a missing-file error on every real PR — nothing in this repo had ever actually run this workflow for real before this pass caught it. Fixed to `pnpm exec vitest --run --coverage`, bypassing the `test` script entirely (only NestJS defines a working `test:cov` alternative; nextjs/vite-react don't, and this job is shared across all three). Verified directly against two live scaffolded projects.

### Infrastructure

- Audit skill internal changelog bumped to 2.10.0 (self-update inventory + FastAPI `python -m pytest` checklist item).
...
- **Universal Git Workflow / fetch-first branch-source section in scaffold AGENTS.md templates.** The embedded AGENTS.md template block in all four stack `source-files.md` files (Next.js, FastAPI, NestJS, Vite+React) and the shared AGENTS.md tail fragment (`shared/harness-kit.md`) emit a `## Git Workflow` section: always fork feature branches from a freshly-pulled `main` (`git fetch -p`, `git pull --ff-only`). Only the universal fetch-first step is hardcoded; a per-repo placeholder comment marks where each deployment documents its own protected-branch route table. A one-line note clarifies this is a fixed hook baseline, not a route table — `block-no-verify.sh` separately hardcodes `main`/`uat`/`develop` as protected branch names, which could otherwise read as contradictory alongside "no develop/uat/staging route is baked in."
- **Full ESLint comment gate in scaffold eslint templates.** The `eslint.config.mjs` templates for the Next.js, Vite+React, and NestJS stacks (in each stack's `config-files.md`) ship the complete comment gate in place of the old non-blocking `'no-inline-comments': 'warn'`: `no-inline-comments` at `error` with an `ignorePattern` (`eslint-|@ts-|prettier-|c8 |istanbul |webpackChunkName`), `sonarjs/no-commented-code` at `error`, the `sonarjs` plugin registered, and `eslint-plugin-sonarjs` (`4.1.0`) added to each stack's `devDependencies`.
- **Deterministic scaffold-parse CI gate (`.github/workflows/scaffold-parse.yml` + `scripts/validate-scaffold-configs.sh`).** Extracts every ```` ```json ```` fenced block from the scaffold `skills/scaffold/*/config-files.md` templates (package.json, tsconfig.json, .prettierrc, components.json, …) and validates each with `jq` — a string-aware pass strips JSONC comments first so tsconfig-style blocks (e.g. Vite+React's `"types": [] // …`) pass while URLs like `"https://…"` are preserved. Also extracts every JS/MJS/JavaScript fence (covering the ESLint comment-gate templates above) and syntax-checks it with `node --check`, reporting `file:line` on failure exactly like the JSON path. A malformed scaffold config or eslint template now fails a deterministic PR check, not only the optional monthly LLM `scaffold-verify` cron. This CI job lives in templateCentral itself and is **not** seeded downstream.
- **`dist-build.yml` CI regression-catcher.** A new job builds `scripts/build-agents-dist.sh`'s output into a clean temp directory (never the repo's own gitignored `dist/`), asserts the build script exits 0, and sanity-checks that every generated (namespaced) `SKILL.md`'s `name:` frontmatter matches its containing folder — no CI job ran or validated this script until now. Also deduplicated a repeated `dist/` line in `.gitignore` (listed once under the Python section and again under a stray "NestJS" section; kept the first — `dist/` remains ignored).

### Security

Hardened the seeded harness guards in `skills/scaffold/shared/harness-kit.md` (mirrored in the OpenCode adapter `adapters/opencode/templatecentral.plugin.js`); these guards are seeded into every scaffolded/migrated project, so the fixes propagate on next scaffold/re-sync:

- **`block-no-verify.sh` now blocks hook-layer bypasses equivalent to `--no-verify`** — a `git commit` invoked with `LEFTHOOK=0` / `LEFTHOOK_EXCLUDE=…` env-var assignment or a `git -c core.hooksPath=…` override (e.g. `/dev/null`) is blocked (exit 2). Existing `--no-verify`/`-n`, protected-branch, force-push and guard-file checks are unchanged; the adapter's `bashCommandReason` mirrors it.
- **`protect-files.sh` CI hard-block extended beyond GitHub** — the CI/CD pipeline hard-block (exit 2) now also covers Azure DevOps (`.azuredevops/**`, `azure-pipelines*.yml`, `azure-pipelines*.yaml`), GitLab (`.gitlab-ci.yml`), Jenkins (`Jenkinsfile`) and `.github/actions/**`, in addition to `.github/workflows/**`. Mirrored in the adapter and in templateCentral's own `scripts/pre-guard.sh` (Tier 2 ASK, per its own threat model). Secret/cert blocks unchanged.
- **`user-prompt-guard` credential regex broadened** — the embedded-credential URL detector (OWASP LLM02) now also catches `postgres://`, `postgresql://`, `mysql://`, `redis://` and `amqp://` URLs with `user:pass@` credentials, not just `mongodb(+srv)://`. Applied to both the `.js` (TS stacks) and `.py` (FastAPI) seeded variants.
- **`settings.json` `.env` Read-deny is now a catch-all** — the hardcoded enumeration of `.env.*` suffixes is replaced with `Read(**/.env.*)` (plus `Read(.env)`/`Read(**/.env)`). Because Claude Code evaluates `deny` before `allow`, the catch-all also read-blocks the two committed `.env.example`/`.env.default` templates; the now-dead paired `Read(**/.env.example)`/`Read(**/.env.default)` `permissions.allow` entries are removed from both seeded `settings.json` blocks (TS and FastAPI) rather than left inert and misleading — fail-closed by design. Templates remain writable via `protect-files.sh` and non-secret (gitleaks-allowlisted); copy their content via bash (`cat .env.example`) if an agent needs it.
- **Seeded `.gitleaks.toml` allowlist no longer exempts the entire `test/` tree.** The template in `shared/harness-kit.md` carried a blanket `'''(^|/)test/.*'''` allowlist path, letting real secrets committed under `test/` escape both pre-commit and CI gitleaks scanning despite the block's own "FALSE POSITIVES only" comment. Removed; the remaining entries (`.env.example`, `.env.default`, lockfiles) are genuine false-positive patterns and are unaffected. Projects that need a specific known-safe fixture allowlisted should add its exact path, not a directory wildcard.
- **`scripts/pre-guard.sh` (templateCentral's own guard; not seeded downstream) no longer fails open when `jq` is missing or `tool_input` fails to parse** — both cases now hard-block (`exit 2`) before any tiering decision, instead of silently yielding an empty `$FILE` and `exit 0`. Also extended Tier 2 (ask) to cover `.claude/settings.local.json` (higher precedence than `settings.json`), `.claude/agents/*`, and `.mcp.json`, none of which had coverage before this fix; the same coverage gap is closed in the seeded `protect-files.sh` (both TS and FastAPI variants), since that guard *is* templated into every scaffolded/migrated project — it parses JSON with `node`/`python3`, not `jq`, so the fail-open-on-missing-interpreter case doesn't apply there. New `scripts/pre-guard.test.sh` exercises this behavior against real stdin payloads — jq-missing fail-closed, malformed-JSON fail-closed, a Tier-1 blocked path, Tier-2 ask paths (including the three new coverage additions), and a normal Tier-3 allow — 18 assertions, wired into CI as a step in `validate-skills.yml`'s `shell-lint` job.
- **SHA-pinned every third-party action in the seeded and illustrative CI templates.** `shared/harness-kit.md`'s seeded `.github/workflows/ci.yml` examples now pin `pnpm/action-setup@a15d269cd4658e1107c09f1fabf4cbd7bd1f308a` (v4.4.0), `actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020` (v4.4.0), `gitleaks/gitleaks-action@ff98106e4c7b2bc287b24eaf42907196329070c7` (v2.3.9, both occurrences), and `actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5` (v4.3.1, all occurrences including the `changelog` job and the FastAPI `quality` job variant) — verified with `grep -n "uses: .*@v[0-9]" skills/scaffold/shared/harness-kit.md` returning zero matches. The illustrative Step B3 CI example is pinned to the same `actions/checkout` SHA, with a comment directing future bumps through the review utility rather than a hand-edit. **Known gap (not fixed in this release):** unrelated unpinned `@v4`/`@v5` action tags remain in `skills/add/mutation-testing/{python,typescript}.md`'s illustrative workflow snippets — surfaced by the repo-wide SHA-pin grep but out of scope for this fix.
- **OpenCode adapter guard clarity + coverage.** `adapters/opencode/README.md`'s "Guard parity" table now has an explicit "Not ported" row and expanded prose noting `user-prompt-guard` (the `UserPromptSubmit` hook running OWASP LLM01/LLM02 checks on every incoming prompt) has no OpenCode port, so an OpenCode user doesn't assume prompt-level protection they don't have — porting the guard itself is a larger, separate effort. Also wired the adapter's existing test suite (`adapters/opencode/hooks.test.mjs`, 37 assertions covering the bash-command and protected-file guard matrix) into CI via a new `opencode-adapter-tests` job in `.github/workflows/validate-skills.yml`, and added `adapters/**` to that workflow's `paths:` filters so adapter-only changes trigger it.

### Changed

- **Single-sourced the AGENTS.md tail — Step G is now authoritative, 6 duplicate copies removed.** The `## AI Harness` / `## Skills Security` / `## Git Workflow` / `## Skill capture` tail was embedded verbatim in all four stack `source-files.md` files and twice in `migrate/general/implementation.md`, alongside the canonical copy in `shared/harness-kit.md`'s Step G, which let the embedded copies silently drift from canonical (they were missing the `Git hooks (lefthook)` / `CI (GitHub Actions)` lines and the entire `## Skill capture` section). Step G now **unconditionally** appends the canonical tail fragment, substituting each stack's `PostToolUse` line from the Per-stack delta table; the 6 embedded copies are deleted and replaced with a one-line pointer to Step G, eliminating ~130+ lines of drift-prone duplication. New scaffolds and migrate re-seeds always emit the current, complete tail from one place.

### Fixed

- **Synced docs and version mirrors to the shipped hard comment gate.** `standards/code-standards/{comments,nextjs,nestjs,vite-react}.md` and `migrate/general/implementation.md` now describe the shipped `no-inline-comments: 'error'` + `sonarjs/no-commented-code: 'error'` gate instead of the old non-blocking `'warn'` wording. The per-stack `## AI Harness` descriptions in `scaffold/{fastapi,nestjs,nextjs,vite-react}/source-files.md` and both embedded AGENTS.md tail copies of `migrate/general/implementation.md` are re-synced to the canonical `shared/harness-kit.md` text (documenting the hook-bypass block and the broader CI hard-block set). Migrated projects now also inherit the universal fetch-first `## Git Workflow` section in both embedded AGENTS.md tail copies of `migrate/general/implementation.md`. Bumped the three release-tracked version mirrors `5.8.0` → `5.9.0`: `.claude/harness.json`, the README version badge, and the `harness.json` literal in `scaffold/shared/harness-kit.md` that stamps every scaffolded project.
- **Removed a dangling "Future Directions" claim in `shared/harness-kit.md`** — "Trace-Driven Evolution" referenced a "disabled trace hook placeholder in `.claude/settings.json`" that the actual seeded settings.json (comment-free JSON) never contained; reworded as an unactivated roadmap item, not an existing seam.
- **Hardened `build-agents-dist.sh` against a bad `OUT` argument.** The script ran `rm -rf "$OUT"` on an unvalidated second CLI arg. Added a sanity check that rejects an empty/`/`/`.`/`..` path and any `OUT` that doesn't contain `dist`, before the destructive `rm`.
- **Fixed `scripts/validate-manifest.sh`'s shell-injection-prone Python invocations.** Several `python3 -c "..."` calls f-string-interpolated file paths and field names directly into the Python source; all are now passed as `sys.argv` values instead (affects `check_json_syntax`, `check_plugin_required_fields`, `check_plugin_extended_fields`, `check_plugin_semver`, `check_skills_path_exists`, `check_marketplace_required_fields`, `check_marketplace_plugin_entries`, `check_marketplace_version_and_source_consistency`, `check_name_consistency`, `check_skill_frontmatter`, `_plugin_version`, `check_repo_harness_version_matches_plugin`).
- **`drift-check` no longer points at a nonexistent skill.** `standards/drift-check/implementation.md` dispatched "the update utility (`skills/review/update/implementation.md`)" on user accept; now dispatches the review utility's update operation (`cat "<skill-dir>/../review/SKILL.md"`).
- **Cross-stack config drift fixes:** `@types/node` `^25.9.1` → `^24` (NestJS, Next.js, Vite+React `config-files.md`, aligning the three TS stacks); `eslint-plugin-react-hooks` `^7.0.0` → `^7.1.1` (Next.js); NestJS's `.prettierrc` gained explicit `semi: true`, `tabWidth: 2`, `printWidth: 100` and `trailingComma` `all` → `es5` to match the other two TS stacks; Vite+React's `package.json` `format`/`format:write` scripts renamed to the cross-stack convention `format` (now `prettier --write .`) / `format:check` (`prettier --check .`).
- **`mutmut` config template fixed to match its current CLI/config surface.** `add/mutation-testing/python.md`'s `pyproject.toml` snippet used `paths_to_mutate`/`tests_dir` keys and a `mutmut run --paths-to-mutate src/ || true` CI step masking failures; now uses `source_paths`/`pytest_add_cli_args_test_selection`, a bare `mutmut run` (no swallowed exit code), and the `.gitignore` entry is `mutants/` (mutmut's actual cache directory) instead of `.mutmut-cache`.
- **Table-render fix in `add/database/{python,typescript}.md`.** The `<skill-dir>` explainer blockquote was inserted between a markdown table's header row and its separator row, breaking table rendering; moved above the table.
- **Form skill TODO stubs now fail loudly instead of silently.** `add/form/{nextjs,vite-react}.md`'s generated `onSubmit` used to `console.log` the payload and show a success toast even though nothing was wired up; it now throws (surfaced via an error toast) with the payload serialized into the message, so an unwired form is visibly broken rather than looking like it works.
- **Fixed a fabricated-password example in `add/database/python/beanie.md`.** The seeded `create_user` example passed a literal `hashed_password="..."` instead of hashing the real submitted password; now excludes `password` from the `model_dump()` payload and sets `hashed_password=hash_password(payload.password)` using the already-referenced `core/security.py` helper.
...
- **README "Other Agent Tools" onboarding** — a clean, numbered OpenCode/OpenChamber quickstart (clone → `build-agents-dist.sh` → register via `skills.paths`, optional guard-plugin step, restart, verify) plus a two-line Codex/Antigravity copy step, all linking to `docs/CROSS-TOOL.md` for depth. `FUTURE.md` §6 gains a per-tool harness-adapter status table + Codex/Antigravity adapter roadmap.

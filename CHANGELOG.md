...
- **Deterministic scaffold-parse CI gate (`.github/workflows/scaffold-parse.yml` + `scripts/validate-scaffold-configs.sh`).** A new PR check extracts every ```` ```json ```` fenced block from the scaffold `skills/scaffold/*/config-files.md` templates (package.json, tsconfig.json, .prettierrc, components.json, …) and validates each with `jq` — a string-aware pass strips JSONC comments first so tsconfig-style blocks (e.g. Vite+React's `"types": [] // …`) pass while URLs like `"https://…"` are preserved. A malformed scaffold config template now fails a deterministic PR check, not only the optional monthly LLM `scaffold-verify` cron. This CI job lives in templateCentral itself and is **not** seeded downstream.

### Security

- **Hardened the seeded harness guards** in `skills/scaffold/shared/harness-kit.md` (mirrored in the OpenCode adapter `adapters/opencode/templatecentral.plugin.js`); these guards are seeded into every scaffolded/migrated project, so the fix propagates on next scaffold/re-sync:
  - **`block-no-verify.sh` now blocks hook-layer bypasses equivalent to `--no-verify`** — a `git commit` invoked with `LEFTHOOK=0` / `LEFTHOOK_EXCLUDE=…` env-var assignment or a `git -c core.hooksPath=…` override (e.g. `/dev/null`) is blocked (exit 2). Existing `--no-verify`/`-n`, protected-branch, force-push and guard-file checks are unchanged. Adapter's `bashCommandReason` mirrors it.
  ...
  - **`settings.json` `.env` Read-deny is now a catch-all** — the hardcoded enumeration of `.env.*` suffixes is replaced with `Read(**/.env.*)` (plus `Read(.env)`/`Read(**/.env)`). Because Claude Code evaluates `deny` before `allow` (a `deny` cannot carry allowlist exceptions), the catch-all also read-blocks the two committed `.env.example`/`.env.default` templates; **no paired `permissions.allow` entries are seeded for them** — such entries would be dead and misleading, so they are intentionally omitted (fail-closed). The templates remain writable via `protect-files.sh` and non-secret (gitleaks-allowlisted); copy their content via bash (`cat .env.example`) if an agent needs it. Applied to both the TS and FastAPI settings blocks.
...
- **`scripts/pre-guard.sh` (templateCentral's own guard; not seeded downstream) no longer fails open when `jq` is missing or `tool_input` fails to parse.** Previously an absent `jq` or malformed JSON silently yielded an empty `$FILE` and `exit 0`, letting every Edit/Write through with the entire tiered guard bypassed. Both cases now hard-block (`exit 2`) before any tiering decision is made. Also extended Tier 2 (ask) to cover `.claude/settings.local.json` (higher precedence than `settings.json` — can disable hooks or grant permissive perms without ever touching the file already being watched), `.claude/agents/*`, and `.mcp.json`, none of which were previously protected. The equivalent coverage gap (missing `settings.local.json`/`.claude/agents/*`/`.mcp.json`) is also closed in the seeded `protect-files.sh` (both TS and FastAPI variants) in `shared/harness-kit.md`, since that guard *is* templated into every scaffolded/migrated project; the seeded guard parses JSON with `node`/`python3`, not `jq`, so the fail-open-on-missing-interpreter case doesn't apply there.
- **Actually completed the SHA-pinning this entry previously claimed was done.** The line below ("Illustrative CI workflow's `actions/checkout@v4` SHA-pinned") pinned only 1 of the 6-7 third-party action references in `shared/harness-kit.md`'s seeded `.github/workflows/ci.yml` examples — `pnpm/action-setup@v4`, `actions/setup-node@v4`, `gitleaks/gitleaks-action@v2` (two occurrences), and two further `actions/checkout@v4` occurrences (the `changelog` job and the FastAPI `quality` job variant) were still on mutable tags, despite the file's own "SHA-pin the actions" policy note. All are now pinned to a specific release SHA with a `# vX.Y.Z` comment, matching the existing style: `pnpm/action-setup@a15d269cd4658e1107c09f1fabf4cbd7bd1f308a` (v4.4.0), `actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020` (v4.4.0), `gitleaks/gitleaks-action@ff98106e4c7b2bc287b24eaf42907196329070c7` (v2.3.9, both occurrences), `actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5` (v4.3.1, both remaining occurrences). Verified with `grep -n "uses: .*@v[0-9]" skills/scaffold/shared/harness-kit.md` returning zero matches. Scope note: this fix covers `harness-kit.md`'s seeded CI template only — a repo-wide grep surfaced unrelated unpinned `@v4`/`@v5` action tags in `skills/add/mutation-testing/{python,typescript}.md`'s illustrative workflow snippets, which are **out of scope for this fix and remain open** (not claimed as closed).
- **Disclosed a guard-parity gap in the OpenCode adapter instead of leaving it implied.** `adapters/opencode/README.md`'s "Guard parity" table covered the Bash/Edit/Write/typecheck hooks but never said `user-prompt-guard` (the `UserPromptSubmit` hook that runs OWASP LLM01 prompt-injection and LLM02 credential-leak detection on every incoming prompt) has no OpenCode port. Added an explicit "Not ported" table row and expanded prose note so an OpenCode user doesn't assume they have prompt-level protection they don't have. Porting the guard itself is a larger, separate effort and is out of scope here.

### Fixed

- **Completed the 5.9.0 rollout (previously half-applied).** Synced the code-standards and migrate docs that still described the old non-blocking `no-inline-comments: 'warn'` nudge to the shipped hard gate (`no-inline-comments: 'error'` + `sonarjs/no-commented-code: 'error'`): `standards/code-standards/{comments,nextjs,nestjs,vite-react}.md` and `migrate/general/implementation.md`. Bumped the three release-tracked version mirrors `5.8.0` → `5.9.0` (`.claude/harness.json`, README version badge, and the `harness.json` literal in `scaffold/shared/harness-kit.md` that stamps every scaffolded project).
- **Completed the 5.9.0 rollout — harness-doc sync (second pass).** Three doc/config-sync gaps the release left half-applied (enforcement layer was already correct):
  - **Re-synced the per-stack `## AI Harness` descriptions** in `scaffold/{fastapi,nestjs,nextjs,vite-react}/source-files.md` and in both embedded AGENTS.md tail copies of `migrate/general/implementation.md`, to the canonical `shared/harness-kit.md` text, so they document the shipped guards (second Bash guard also blocks hook-layer bypasses `LEFTHOOK=0`/`git -c core.hooksPath=…`; PreToolUse blocks the full CI/CD set — `.github/actions/`, `.azuredevops/`, `azure-pipelines*.y[a]ml`, `.gitlab-ci.yml`, `Jenkinsfile` — not just `.github/workflows/`) instead of the pre-5.9.0 wording. Each copy's per-stack `PostToolUse` typecheck line is unchanged.
  - **Migrate Git-Workflow parity** — added the universal fetch-first `## Git Workflow` section (with the per-repo route-table placeholder comment) to both embedded AGENTS.md tail copies in `migrate/general/implementation.md`, so migrated projects inherit the same section the four scaffold stacks and the shared tail already emit.
  - **Removed the dead `.env` `permissions.allow` entries** — `Read(**/.env.example)`/`Read(**/.env.default)` are dropped from both seeded `settings.json` blocks in `harness-kit.md`; the `Read(**/.env.*)` catch-all deny wins under Claude Code deny-precedence, so the allow entries were inert and misleading. Kept the fail-closed catch-all and documented that `.env.example` is intentionally not agent-readable (copy via bash).
- **Single-sourced the AGENTS.md tail — Step G is now authoritative, 6 duplicate copies removed.** The `## AI Harness` / `## Skills Security` / `## Git Workflow` / `## Skill capture` tail was embedded verbatim in all four stack `source-files.md` files and twice in `migrate/general/implementation.md`, alongside the canonical copy in `shared/harness-kit.md`'s Step G. Because every template pre-embedded the tail, Step G's old "append only if `## AI Harness` is absent" check was a perpetual no-op, so the embedded copies silently drifted from canonical as it evolved (confirmed drift: the embedded copies were missing the `Git hooks (lefthook)` / `CI (GitHub Actions)` lines added in a prior release and the entire `## Skill capture` section added further back — neither had ever propagated past `shared/harness-kit.md`). Step G now **unconditionally** appends the canonical tail fragment, substituting each stack's `PostToolUse` line from the Per-stack delta table; the 6 embedded copies are deleted and replaced with a one-line pointer to Step G, eliminating ~130+ lines of drift-prone duplication. New scaffolds and migrate re-seeds now always emit the current, complete tail from one place.
- **Two smaller `harness-kit.md` doc fixes:**
  - **Dangling "Future Directions" reference removed** — "Trace-Driven Evolution" claimed a "disabled trace hook placeholder in `.claude/settings.json`" that the actual seeded settings.json (comment-free JSON) never contained. Reworded as an unactivated roadmap item, not an existing seam.
  - **Illustrative CI workflow's `actions/checkout@v4` SHA-pinned** — the Step B3 example carried a mutable `@v4` tag with a "SHA-pin in production" comment that was never itself pinned. Pinned to `34e114876b0b11c390a56381ad16ebd13914f8d5` (`v4.3.1`, current as of this release) with a comment directing future bumps through the review utility rather than a hand-edit.
- **Closed the last 5.9.0 audit gap: scaffold-parse now covers JS config fences, plus a new dist-build CI regression-catcher.** `scripts/validate-scaffold-configs.sh` extracted and `jq`-validated only ` ```json ` fences from `skills/scaffold/*/config-files.md`, leaving the headline 5.9.0 change — the ESLint comment gate, which lives in the ` ```js `/` ```mjs `/` ```javascript ` `eslint.config.mjs` fences — unchecked by any deterministic gate (only the optional monthly LLM `scaffold-verify` cron covered it). The script now also extracts every JS/MJS/JavaScript fence and syntax-checks it with `node --check`, reporting `file:line` on failure exactly like the JSON path; `.github/workflows/scaffold-parse.yml`'s job/step names updated from "JSON" to "JSON/JS". Added `.github/workflows/dist-build.yml`: a new CI job that builds `scripts/build-agents-dist.sh`'s output into a clean temp directory (never the repo's own gitignored `dist/`), asserts the build script exits 0, and sanity-checks that every generated (namespaced) `SKILL.md`'s `name:` frontmatter matches its containing folder — previously nothing in CI ran or validated this script. Also deduplicated a repeated `dist/` line in `.gitignore` (`dist/` was listed once under the Python section and again under a stray "NestJS" section; kept the first, `dist/` remains ignored).
- **Wired the OpenCode adapter's test suite into CI.** `adapters/opencode/hooks.test.mjs` (37 assertions covering the bash-command and protected-file guard matrix) existed but no workflow ran it. Added an `opencode-adapter-tests` job to `.github/workflows/validate-skills.yml` running `node adapters/opencode/hooks.test.mjs`, and added `adapters/**` to that workflow's push/pull_request `paths:` filters so adapter-only changes trigger it.
- **Clarified the hook-baseline vs. per-repo-route wording in the seeded Git Workflow doc.** The seeded `## Git Workflow` AGENTS.md section says "no develop/uat/staging route is baked in," while `block-no-verify.sh` separately hardcodes `main`/`uat`/`develop` as protected branch names — not actually contradictory (a fixed hook baseline vs. a per-repo route table) but confusing side by side. Added a one-line clarifying note in `shared/harness-kit.md`'s Git Workflow section.
...
- **Added a behavioral test for `pre-guard.sh`'s tiering/fail-closed logic** (the bf6e4ca fix), previously covered only by shellcheck/`bash -n` syntax checks, not by anything exercising its actual behavior. New `scripts/pre-guard.test.sh` runs the script against real stdin payloads and asserts exit codes / output for: jq-missing fail-closed, malformed-JSON fail-closed, a Tier-1 blocked path, Tier-2 ask paths (including the `settings.local.json`/`.claude/agents/*`/`.mcp.json` coverage added by that fix), and a normal Tier-3 allow — 18 assertions, all passing. Wired into CI as a new step in `validate-skills.yml`'s `shell-lint` job.
- **Correction to this same changelog entry:** the line above previously read "**This closes the full 5.9.0 audit backlog** — B1–B16 plus the pre-guard, drift-check, and config-drift items surfaced in the supplementary sweeps — with no outstanding items." That was false with respect to SHA-pinning — see the SHA-pinning bullet under Security above, which fixes the underlying gap and is grep-verified. This entry now also closes five further items from a follow-up audit (opencode test wiring, opencode guard-gap disclosure, Git Workflow wording clarity, `build-agents-dist.sh` rm -rf hardening, and the new `pre-guard.sh` behavioral test). One item remains genuinely open and is **not** claimed as closed: unrelated unpinned `@v4`/`@v5` action tags in `skills/add/mutation-testing/{python,typescript}.md`'s illustrative workflow snippets, surfaced by this pass's repo-wide SHA-pin grep but out of scope for this fix.

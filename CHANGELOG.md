...
- **Completed the 5.9.0 rollout — harness-doc sync (second pass).** Three doc/config-sync gaps the release left half-applied (enforcement layer was already correct):
  - **Re-synced the four per-stack `## AI Harness` descriptions** in `scaffold/{fastapi,nestjs,nextjs,vite-react}/source-files.md` to the canonical `shared/harness-kit.md` text, so they document the shipped guards (second Bash guard also blocks hook-layer bypasses `LEFTHOOK=0`/`git -c core.hooksPath=…`; PreToolUse blocks the full CI/CD set — `.github/actions/`, `.azuredevops/`, `azure-pipelines*.y[a]ml`, `.gitlab-ci.yml`, `Jenkinsfile` — not just `.github/workflows/`) instead of the pre-5.9.0 wording. Each stack's per-stack `PostToolUse` typecheck line is unchanged.
